...
# 模型搭建--版本2
model = Sequential([
    LSTM(units=32, input_shape=(seq_len, 1),activation='selu', return_sequences=False), #1
    # LSTM(units=26, input_shape=(seq_len, 1),return_sequences=True), #1
    # LSTM(units=23,return_sequences=True),
    # LSTM(units=17,activation='selu', return_sequences=False),
    #layers.Dropout(0.3),
    #LSTM(units=16,kernel_initializer='he_uniform', kernel_regularizer=regularizers.l2(0.01), activation='selu'),
    Dense(1)
])
